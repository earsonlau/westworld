...
#
# 方法一：哈希表（空间复杂度不符合要求）
# 按照刚才我们读例子的思路，其实我们只需从最小的正整数 1 开始，
# 依次判断2、3 、4 直到数组的长度 N 是否在数组中。
#
# 如果当前考虑的数不在这个数组中，我们就找到了这个缺失的最小正整数。
# 由于我们需要依次判断某一个正整数是否在这个数组里，我们可以先把这个数组中所有的元素放进哈希表。
# 接下来再遍历的时候，就可以以 O(1)的时间复杂度判断某个正整数是否在这个数组。
...
# 方法二：二分查找（时间复杂度不符合要求）
# 根据刚才的分析，这个问题其实就是要我们查找一个元素，而查找一个元素，
# 如果是在有序数组中查找，会快一些。因此我们可以将数组先排序，
# 再使用二分查找法从最小的正整数 1 开始查找，找不到就返回这个正整数；
...
# 方法三：将数组视为哈希表
# 由于题目要求我们“只能使用常数级别的空间”，而要找的数一定在 [1, N + 1] 左闭右闭（这里 N 是数组的长度）
# 这个区间里。
...
class Solution:
    ...
    def firstMissingPositive(self, nums: List[int]) -> int:
        size = len(nums)
        for i in range(size):
            # 先判断这个数字是不是索引，然后判断这个数字nums[i]是不是放在了正确的地方nums[nums[i]-1]
